...
typedef VipsOperationClass VipssRGB2HSVClass;

G_DEFINE_TYPE( VipssRGB2HSV, vips_sRGB2HSV, VIPS_TYPE_OPERATION );

/* Convert a buffer of 8-bit pixels.
 */
static void
vips_sRGB2HSV_line_8( float * restrict q, VipsPel * restrict p,
	int extra_bands, int width )
{
	...
	for( i = 0; i < width; i++ ) {
		...
		vips_col_sRGB2HSV_8( r, g, b, &H, &S, &V );
 ...
 */
static void
vips_sRGB2HSV_line_16( float * restrict q, unsigned short * restrict p,
	int extra_bands, int width )
{
	...
	for( i = 0; i < width; i++ ) {
		...
		vips_col_sRGB2HSV_16( r, g, b, &H, &S, &V );
...
static int
vips_sRGB2HSV_gen( VipsRegion *or,
	void *seq, void *a, void *b, gboolean *stop )
{
	...
	VIPS_GATE_START( "vips_sRGB2HSV_gen: work" );
	...
	for( y = 0; y < r->height; y++ ) {
		...
		if( in->BandFmt == VIPS_FORMAT_UCHAR )
			vips_sRGB2HSV_line_8( q, p,
				in->Bands - 3, r->width );
		else
			vips_sRGB2HSV_line_16( q, (unsigned short *) p,
				in->Bands - 3, r->width );
	}

	VIPS_GATE_STOP( "vips_sRGB2HSV_gen: work" );

	return( 0 );
}

static int
vips_sRGB2HSV_build( VipsObject *object )
{
	VipsObjectClass *class = VIPS_OBJECT_GET_CLASS( object ); 
	VipssRGB2HSV *sRGB2HSV = (VipssRGB2HSV *) object;
	...
	if( VIPS_OBJECT_CLASS( vips_sRGB2HSV_parent_class )->
		build( object ) )
		return( -1 );
	...
	out->Type = VIPS_INTERPRETATION_HSV;
	out->BandFmt = VIPS_FORMAT_FLOAT;

	if( vips_image_generate( out,
		vips_start_one, vips_sRGB2HSV_gen, vips_stop_one,
		in, sRGB2HSV ) ) {
		g_object_unref( out );
		return( -1 );
	}
...
vips_sRGB2HSV_class_init( VipssRGB2HSVClass *class )
{
	...
	object_class->description = _( "convert an sRGB image to HSV" );
	object_class->build = vips_sRGB2HSV_build;
	...
	VIPS_ARG_IMAGE( class, "in", 1, 
		...
		G_STRUCT_OFFSET( VipssRGB2HSV, in ) );

	VIPS_ARG_IMAGE( class, "out", 100, 
		_( "Output" ), 
		_( "Output image" ),
		VIPS_ARGUMENT_REQUIRED_OUTPUT, 
		G_STRUCT_OFFSET( VipssRGB2HSV, out ) );
 ...
 *
 * Convert an sRGB image to HSV. The input image can be 8 or 16-bit
 * unsigned int.
 *
